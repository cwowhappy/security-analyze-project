...
const chartOption = computed(() => {
  ...
  return {
    ...
    title: { text: '成本费用结构', left: 'center', textStyle: { color: '#E8EAED', fontSize: 14 } },
    tooltip: {
      trigger: 'axis',
    ...
    series: [
      { name: '销售费用', type: 'bar', stack: 'expense', data: props.metrics.map(m => m.saleExpense), yAxisIndex: 0, itemStyle: { color: NEON_COLORS[0] } },
      { name: '管理费用', type: 'bar', stack: 'expense', data: props.metrics.map(m => m.manageExpense), yAxisIndex: 0, itemStyle: { color: NEON_COLORS[3] } },
      { name: '研发费用', type: 'bar', stack: 'expense', data: props.metrics.map(m => m.researchExpense), yAxisIndex: 0, itemStyle: { color: NEON_COLORS[4] } },
      { name: '财务费用', type: 'bar', stack: 'expense', data: props.metrics.map(m => m.financeExpense), yAxisIndex: 0, itemStyle: { color: NEON_COLORS[6] } },
      { name: '期间费用率', type: 'line', data: props.metrics.map(m => m.periodExpenseRate), yAxisIndex: 1, smooth: true, itemStyle: { color: NEON_COLORS[2] } },
    ],
  }
})
</script>
