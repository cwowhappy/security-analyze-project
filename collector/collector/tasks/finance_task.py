...
class FinanceTask:
        ...
        try:
            ...
            for item in stock_list:
                ...
                if not stock_code:
                    continue
                try:
                    c, u = self._collect_by_stock_code(stock_code)
                    created += c
                    updated += u
    ...
    def run_by_stock_code(self, stock_code: str):
        """按股票代码采集指定公司的全部财务报告"""
        ...
        try:
            created, updated = self._collect_by_stock_code(stock_code)
            rows = created + updated
            if self.monitor:
    ...
    def _collect_by_stock_code(self, stock_code: str) -> tuple[int, int]:
        """采集单个公司的财务报告，返回 (created, updated)"""
        market = self.source.infer_market(stock_code)
        symbol = f"{market}{stock_code}"

        # 采集三张报表
        balance_df = self.source.get_balance_sheet(symbol)
        profit_df = self.source.get_profit_sheet(symbol)
        cashflow_df = self.source.get_cash_flow_sheet(symbol)
